...
/* The four #s below for the UART to the communication between the HMI ECU & CONTROL ECU and can be changed as we want but
 * can't be the same.
 * We use the polling method with the UART because every one of the ECU depend on the other ECU. By meaning, the CONTROL
 * ECU depend on the choice that the user will make in the HMI ECU and the HMI ECU depends on the decision of the
 * CONTROL ECU.
 * */
...
/*The #s below is made to make the code readable for anyone
 * the values can be anything but the values must be unique (not the same)
 * every value describe a different behaviour
 * */
...
/* Very important variable that will make the choices and decisions of the whole system*/
uint8 choice=FALSE;
...
void getSendPassword(){
	...
	LCD_moveCursor(1,5);
	/* Looping 5-times to the password size in order to take the whole character of the password from the user*/
	for(count=0 ; count<PASSWORD;count++){
		password[count]=KEYPAD_getPressedKey();
		LCD_displayCharacter('*');
		/*Why delay ??
		 * --> micro-controller speed is very higher than human speed so we made this delay for that purpose
		 * to not input more than one character
		 * */
		_delay_ms(400);
	}
	/* '=' is equivalent to enter. the user must enter that key to send the password to the CONTROL ECU */
	 ...
	 * */
	_delay_ms(400);
	/* Looping for the password size to send all the characters to the CONTROL ECU*/
	for(count=0;count<PASSWORD;count++){
		/* check if the HMI ECU is ready */
		UART_sendByte(ARE_MC2_READY);
		/* Looping until HMI ECU send that it is ready which is MC2_RREADY*/
...
int main(){
	 ...
	 * */
	TIMER_configType configtype1={TIMER1,COMPARE_MODE_TIMER1,F_CPU_1024,0,0,46875};
	 ...
	 *
	 * */
	TIMER_configType configtype2={TIMER1,COMPARE_MODE_TIMER1,F_CPU_64,0,0,62500};
